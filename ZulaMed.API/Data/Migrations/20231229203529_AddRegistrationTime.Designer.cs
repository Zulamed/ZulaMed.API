// <auto-generated />
using System;
using System.Collections.Generic;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Mux.Csharp.Sdk.Model;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using ZulaMed.API.Data;
using ZulaMed.API.Domain.Video;

#nullable disable

namespace ZulaMed.API.Data.Migrations
{
    [DbContext(typeof(ZulaMedDbContext))]
    [Migration("20231229203529_AddRegistrationTime")]
    partial class AddRegistrationTime
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "live_stream_status", new[] { "active", "idle", "disabled" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "video_status", new[] { "waiting_for_upload", "getting_processed", "ready", "cancelled" });
            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("PlaylistVideo", b =>
                {
                    b.Property<Guid>("PlaylistsId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("VideosId")
                        .HasColumnType("uuid");

                    b.HasKey("PlaylistsId", "VideosId");

                    b.HasIndex("VideosId");

                    b.ToTable("PlaylistVideo");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Accounts.HospitalAccount.HospitalAccount", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<string>("AccountAddress")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountHospital")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountPhone")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountPostCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("UserId");

                    b.ToTable("HospitalAccount");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Accounts.PersonalAccount.PersonalAccount", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<DateOnly>("AccountBirthDate")
                        .HasColumnType("date");

                    b.Property<string>("AccountCareerStage")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountDepartment")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountGender")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountInstitute")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountProfessionalActivity")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountRole")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountSpecialty")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountTitle")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<List<string>>("PlacesOfWork")
                        .IsRequired()
                        .HasColumnType("text[]");

                    b.HasKey("UserId");

                    b.ToTable("PersonalAccount");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Accounts.UniversityAccount.UniversityAccount", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.Property<string>("AccountAddress")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountPhone")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountPostCode")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("AccountUniversity")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("UserId");

                    b.ToTable("UniversityAccount");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Comments.Comment", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("RelatedVideoId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("SentAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("SentById")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("RelatedVideoId");

                    b.HasIndex("SentById");

                    b.ToTable("Comment");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Comments.Reply", b =>
                {
                    b.Property<Guid>("ParentCommentId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ReplyCommentId")
                        .HasColumnType("uuid");

                    b.HasKey("ParentCommentId", "ReplyCommentId");

                    b.HasIndex("ReplyCommentId");

                    b.ToTable("Reply");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Dislike.Dislike<ZulaMed.API.Domain.Video.Video>", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("ParentId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("DislikedById")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("DislikedAt")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id", "ParentId", "DislikedById");

                    b.HasIndex("DislikedById");

                    b.HasIndex("ParentId");

                    b.ToTable("Dislike<Video>");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Like.Like<ZulaMed.API.Domain.Video.Video>", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("ParentId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("LikedById")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("LikedAt")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id", "ParentId", "LikedById");

                    b.HasIndex("LikedById");

                    b.HasIndex("ParentId");

                    b.ToTable("Like<Video>");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.LiveStream.LiveStream", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("MuxStreamId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("PlaybackId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("RelatedVideoId")
                        .HasColumnType("uuid");

                    b.Property<LiveStreamStatus>("Status")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("live_stream_status")
                        .HasDefaultValue(LiveStreamStatus.Idle);

                    b.Property<string>("StreamKey")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("RelatedVideoId");

                    b.ToTable("LiveStream");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Playlist.Playlist", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("OwnerId")
                        .HasColumnType("uuid");

                    b.Property<string>("PlaylistDescription")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("PlaylistName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("OwnerId");

                    b.ToTable("Playlist");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Subscriptions.Subscription", b =>
                {
                    b.Property<Guid>("SubscriberId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("SubscribedToId")
                        .HasColumnType("uuid");

                    b.HasKey("SubscriberId", "SubscribedToId");

                    b.HasIndex("SubscribedToId");

                    b.ToTable("Subscription");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.User.User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("City")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Country")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<bool>("HistoryPaused")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsVerified")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false);

                    b.Property<string>("Login")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("PhotoUrl")
                        .HasColumnType("text");

                    b.Property<DateTime>("RegistrationTime")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("SubscriberCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<string>("Surname")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("Login")
                        .IsUnique();

                    b.ToTable("User");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Video.Video", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("PublisherId")
                        .HasColumnType("uuid");

                    b.Property<string>("VideoDescription")
                        .HasColumnType("text");

                    b.Property<int>("VideoDislike")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<int>("VideoLike")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0);

                    b.Property<DateTime>("VideoPublishedDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<VideoStatus>("VideoStatus")
                        .HasColumnType("video_status");

                    b.Property<string>("VideoThumbnail")
                        .HasColumnType("text");

                    b.Property<string>("VideoTimelineThumbnail")
                        .HasColumnType("text");

                    b.Property<string>("VideoTitle")
                        .HasColumnType("text");

                    b.Property<string>("VideoUrl")
                        .HasColumnType("text");

                    b.Property<long>("VideoView")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasDefaultValue(0L);

                    b.HasKey("Id");

                    b.HasIndex("PublisherId");

                    b.ToTable("Video");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.ViewHistory.ViewHistory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ViewedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ViewedById")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ViewedVideoId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ViewedById");

                    b.HasIndex("ViewedVideoId");

                    b.ToTable("ViewHistory");
                });

            modelBuilder.Entity("PlaylistVideo", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.Playlist.Playlist", null)
                        .WithMany()
                        .HasForeignKey("PlaylistsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ZulaMed.API.Domain.Video.Video", null)
                        .WithMany()
                        .HasForeignKey("VideosId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Accounts.HospitalAccount.HospitalAccount", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "User")
                        .WithOne("HospitalAccount")
                        .HasForeignKey("ZulaMed.API.Domain.Accounts.HospitalAccount.HospitalAccount", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Accounts.PersonalAccount.PersonalAccount", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "User")
                        .WithOne("PersonalAccount")
                        .HasForeignKey("ZulaMed.API.Domain.Accounts.PersonalAccount.PersonalAccount", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Accounts.UniversityAccount.UniversityAccount", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "User")
                        .WithOne("UniversityAccount")
                        .HasForeignKey("ZulaMed.API.Domain.Accounts.UniversityAccount.UniversityAccount", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Comments.Comment", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.Video.Video", "RelatedVideo")
                        .WithMany("Comments")
                        .HasForeignKey("RelatedVideoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ZulaMed.API.Domain.User.User", "SentBy")
                        .WithMany()
                        .HasForeignKey("SentById")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RelatedVideo");

                    b.Navigation("SentBy");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Comments.Reply", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.Comments.Comment", "ParentComment")
                        .WithMany()
                        .HasForeignKey("ParentCommentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ZulaMed.API.Domain.Comments.Comment", "ReplyComment")
                        .WithMany()
                        .HasForeignKey("ReplyCommentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ParentComment");

                    b.Navigation("ReplyComment");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Dislike.Dislike<ZulaMed.API.Domain.Video.Video>", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "DislikedBy")
                        .WithMany()
                        .HasForeignKey("DislikedById")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ZulaMed.API.Domain.Video.Video", "Parent")
                        .WithMany("Dislikes")
                        .HasForeignKey("ParentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("DislikedBy");

                    b.Navigation("Parent");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Like.Like<ZulaMed.API.Domain.Video.Video>", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "LikedBy")
                        .WithMany()
                        .HasForeignKey("LikedById")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ZulaMed.API.Domain.Video.Video", "Parent")
                        .WithMany("Likes")
                        .HasForeignKey("ParentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("LikedBy");

                    b.Navigation("Parent");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.LiveStream.LiveStream", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.Video.Video", "RelatedVideo")
                        .WithMany()
                        .HasForeignKey("RelatedVideoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RelatedVideo");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Playlist.Playlist", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "Owner")
                        .WithMany()
                        .HasForeignKey("OwnerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Subscriptions.Subscription", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "SubscribedTo")
                        .WithMany("Subscribers")
                        .HasForeignKey("SubscribedToId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ZulaMed.API.Domain.User.User", "Subscriber")
                        .WithMany("Subscriptions")
                        .HasForeignKey("SubscriberId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SubscribedTo");

                    b.Navigation("Subscriber");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Video.Video", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "Publisher")
                        .WithMany("Videos")
                        .HasForeignKey("PublisherId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Publisher");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.ViewHistory.ViewHistory", b =>
                {
                    b.HasOne("ZulaMed.API.Domain.User.User", "ViewedBy")
                        .WithMany()
                        .HasForeignKey("ViewedById")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ZulaMed.API.Domain.Video.Video", "ViewedVideo")
                        .WithMany("ViewHistories")
                        .HasForeignKey("ViewedVideoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ViewedBy");

                    b.Navigation("ViewedVideo");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.User.User", b =>
                {
                    b.Navigation("HospitalAccount");

                    b.Navigation("PersonalAccount");

                    b.Navigation("Subscribers");

                    b.Navigation("Subscriptions");

                    b.Navigation("UniversityAccount");

                    b.Navigation("Videos");
                });

            modelBuilder.Entity("ZulaMed.API.Domain.Video.Video", b =>
                {
                    b.Navigation("Comments");

                    b.Navigation("Dislikes");

                    b.Navigation("Likes");

                    b.Navigation("ViewHistories");
                });
#pragma warning restore 612, 618
        }
    }
}
